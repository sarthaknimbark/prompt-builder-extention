...
* For complex problems, think step-by-step internally before answering`;
  ...
  async function load() {
    ...
    settingsRows.forEach((row) => {
      let val = row.value;
      if (row.key === "proxyEndpoint" && typeof val === "string" && (val.includes(":8789") || val.includes("promptforge-proxy.onrender.com"))) {
        val = "http://localhost:8787/api/chat";
        db.put("settings", { key: row.key, value: val });
      }
      state.settings[row.key] = val;
    });
      ...
      try {
        ...
        if (stored) {
          for (const key of Object.keys(stored)) {
            let val = stored[key];
            if (key === "proxyEndpoint" && typeof val === "string" && (val.includes(":8789") || val.includes("promptforge-proxy.onrender.com"))) {
              val = "http://localhost:8787/api/chat";
              try {
                await chrome.storage.local.set({ proxyEndpoint: val });
              } catch (setErr) {
  ...
  function nav() {
    ...
    return `
      <aside class="sidebar">
        <div class="brand">
          <div class="brand-title">Prompt Builder</div>
  ...
  function renderDashboard() {
    ...
    shell(`
      <div class="page">
        ...
        ${state.error ? `<div class="warning">${escapeHtml(state.error)}</div>` : ""}
        <div class="grid">
  ...
  async function enhancePrompt() {
    ...
    const compiled = compilePrompt(state.activePrompt);
    if (!compiled.trim()) {
      state.error = "Add prompt content before enhancing.";
      render();
      return;
    }
    if (state.settings.apiMode === "direct" && !state.settings.apiKey) {
      state.error = "Add an API key in Settings before enhancing prompts.";
      render();
      return;
    }
    if (state.settings.apiMode === "proxy" && !state.settings.proxyEndpoint) {
      state.error = "Add your hosted backend proxy URL in Settings before enhancing prompts.";
      render();
      return;
    }
...
${compiled}`
      ...
      state.error = `LLM enhancement failed: ${error.name === "AbortError" ? "Enhancement timed out after 30 seconds." : error.message}. Please check if the proxy server is running (e.g. 'npm run start:proxy') or check your API settings.`;
    } finally {
      window.clearTimeout(timeout);
